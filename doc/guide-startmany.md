...
2. [Sending 1000 XMCC to new wallet addresses.](#option2)
...
Just like setting up a standard MN. Send exactly 1000 XMCC to each new address created above.
...
## What about the monoeci.conf file?

If you are using a `masternode.conf` file you no longer need the `monoeci.conf` file. The exception is if you need custom settings (_thanks oblox_). In that case you **must** remove `masternode=1` from local `monoeci.conf` file. This option should be used only to start local Hot masternode now.
